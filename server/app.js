...
app.get('/api/seed-menu', async (req, res) => {
  try {
    ...
    const retiredNames = ['Chocolate Walnut Brownie', 'Choco Lava Brownie', 'Nutella Brownie', 'Oreo Brownie', 'Walnut Brownie'];
    ...
    res.json({ success: true, message: `${synced.length} products synced, ${retiredNames.length} retired names removed`, products: synced.map(p => p.name) });
  } catch (error) {
